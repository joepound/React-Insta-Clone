...
class App extends Component {
  ...
  togglePostHeart(e) {
    ...
    const postQueryResultsIndex = this.state.postQueryResults.findIndex(
      post => post._id === e.currentTarget.dataset._id
    );
    const likeIndex = this.state.posts[postIndex].likes.findIndex(
      liker => liker === this.state.currentUser
    );
  ...
  toggleCommentHeart(e) {
    ...
    const postCommentIndex = this.state.posts[postIndex].comments.findIndex(
      comment => comment._id === e.currentTarget.dataset.comment_id
    );
    const postLikeIndex = this.state.posts[postIndex].comments[
      postCommentIndex
    ].likes.findIndex(liker => liker === this.state.currentUser);

    const postQueryResultsIndex = this.state.postQueryResults.findIndex(
      post => post._id === e.currentTarget.dataset.post_id
    );
    const postQueryResultsCommentIndex = this.state.postQueryResults[
      postQueryResultsIndex
    ].comments.findIndex(
      comment => comment._id === e.currentTarget.dataset.comment_id
    );
    const postQueryResultsLikeIndex = this.state.postQueryResults[
      postQueryResultsIndex
  ...
  handleChange = e => {
    switch (e.currentTarget.name) {
      ...
      case "comment-input":
        const postIndex = this.state.postQueryResults.findIndex(
          post => post._id === e.currentTarget.dataset._id
        );
